...
export class JwtProvider {
    ...
    async createToken(account:string):Promise<string> {
        ...
        const token = jwt.sign({account}, APP_CONFIG.APP_SECRET, { expiresIn }).slice(0,16);
        await this.redisProvider.set(REDIS_SCHEMA.JWT_TOKEN+':'+account,token);
        return token
    }

    async validateToken(signData:SignData): Promise<boolean> {
        const accountToken=await this.redisProvider.get(REDIS_SCHEMA.JWT_TOKEN+':'+signData.account);
